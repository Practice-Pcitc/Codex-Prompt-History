...
function submit() {
  const projectId = form.projectKey.startsWith("id:") ? form.projectKey.slice(3) : undefined;
  const projectName = form.projectKey.startsWith("name:") ? form.projectKey.slice(5) : undefined;
...
<template>
  <section class="filter-bar">
    ...
    <el-input
      ...
      @keyup.enter="submit"
    />
    <el-input v-model="form.sessionId" clearable placeholder="Session ID" @keyup.enter="submit" />
    <el-date-picker
      class="filter-date"
      v-model="form.range"
      type="datetimerange"
